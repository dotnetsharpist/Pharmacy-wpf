...
/// <summary>
/// Interaction logic for MedicineUpdateWindow.xaml
/// </summary>
public partial class MedicineUpdateWindow : Window
{
    public Medicine Medicine { get; set; }
    private readonly IMedicineRepository _medicineRepository;
    public MedicineUpdateWindow()
    {
        InitializeComponent();
    ...
    public void SetData(Medicine medicine)
    {
        ...
        string imgPath = medicine.image_path;
        ImgBImage.ImageSource = new BitmapImage(new Uri(imgPath, UriKind.Relative));
        cmbBoxCategory.Text = medicine.MedCategory;
        tbPrice.Text = medicine.Price;
        dpCreatAt.Text = medicine.CreatedAT.ToLongDateString();
        dpEndAt.Text = medicine.EndTime.ToLongDateString();
        rbDescription.Document.Blocks.Clear();
        rbDescription.Document.Blocks.Add(new Paragraph(new Run(medicine.Description)));
    }
    ...
    private async void btnSave_Click(object sender, RoutedEventArgs e)
    {
        ...
        var result = await _medicineRepository.UpdateAsync(Medicine.MedId,m);
        if (result > 0)
        {
    ...
    {
        var medicinies = await _medicineRepository.GetAllAsync(new Utils.PaginationParams()
        {
            PageNumber = 1,
            PageSize = 100
        });
